...
module.exports = async function (req, res, next) {
  ...
  if (req.headers['x-approuter-authorization'].startsWith('Basic')){
    ...
    try {
      ...
      return handleSession(req,tokenResponse.tokenDecoded, tokenResponse.accessToken,tracer,{credentials:ownCredentials}, next);
    } catch (err){
    ...
    xssec.createSecurityContext(tokenValue, config, service.type, async function (err) {
      if (err) {
        ...
        return handleSession(req, tokenDecoded, tokenValue, tracer,service, next);
      }
    });
  }
};
...
async function handleSession(req, tokenDecoded, tokenValue,tracer,service, next){
